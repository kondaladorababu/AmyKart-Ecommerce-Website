    ...
    return (
        <div className='expander-container' onClick={() => handleToggle(filterType.id)}  >
            ...
            <div className='container-right'>
                {/* if selectedFilters[filterType.id] is not empty,(any specific filter items are not there['nike', 'puma']) then show the length of the array */}
                {selectedFilters[filterType.id] && selectedFilters[filterType.id].length > 0 && (
